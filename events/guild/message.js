...
const { Client } = require("discord.js"); //loading config file with token and prefix
const settings = require("../../botconfig/settings.json"); //loading settings file with the settings
const ee = require("../../botconfig/embed.json"); //Loading all embed settings like color footertext and icon ...
const Discord = require("discord.js");  //this is the official discord.js wrapper for the Discord Api, which we use! //db quick.db
...
module.exports = async (client, message) => {
  try {
    ...
    //Jika command ini tidak ada di folder commands
    if (!command) command = client.commands.get(client.aliases.get(cmd));
    //Jika command ini tidakvalid
    if (command) {
      //Jika ingin mengecek command ini sedang cooldown atau tidak.
      ...
      try {
        ...
        //nsfw require
        if (command.nsfw && !message.channel.nsfw)  return message.channel.send({ embed: new Discord.MessageEmbed()
              .setColor(ee.wrongcolor)
              .setFooter(ee.footertext, ee.footericon)
              .setTitle(replacemsg(settings.messages.nsfw))
          }).then(msg => msg.delete({
        ...
        if (command.memberpermissions && command.memberpermissions.length > 0 && !message.member.hasPermission(command.memberpermissions)) {
          return message.channel.send({ embed: new Discord.MessageEmbed()
              .setColor(ee.wrongcolor)
              .setFooter(ee.footertext, ee.footericon)
              .setTitle(replacemsg(settings.messages.memberpermissions))
          }).then(msg => msg.delete({
            timeout: settings.timeout.notallowed_to_exec_cmd.memberpermissions
          }).catch((e) => {}));
        }
