...
module.exports = {
    name: "rule34", //the command name for execution & for helpcmd [OPTIONAL]
    category: "fun", //the command category for helpcmd [OPTIONAL]
    aliases: ["r34"], //the command aliases for helpcmd [OPTIONAL]
    cooldown: 5, //the command cooldown for execution & for helpcmd [OPTIONAL]
    usage: "rule34 [args][args]", //the command usage for helpcmd [OPTIONAL]
    description: "You know what it is right?", //the command description for helpcmd [OPTIONAL]
    ...
    } else if (isNumber(args.join(' '))) {
        for (let i = 0; i < args.join(' '); i++) {
        axios.get('https://api.waifu.pics/nsfw/waifu')
        .then((res) =>{
        //console.log(res.data.url)
        console.log('=======> WAIFUPICS API RESULT');
